...
void	Voronoi::RemoveParabola(VEvent * e)
{
	...
	if(p0 == p2) std::cout << "error - same focus point\n";
...
/*
Событие точки — это попадание ЗП на один из сайтов, поэтому мы создаём
новую параболу, соответствующую данному сайту, а также добавляются две
контрольные точки (break points)
*/
...
/*
 вершина диаграммы Вороного всегда лежит на пересечении ровно трёх рёбер
 диаграммы, и есть следствие из этой теоремы, которое гласит, что вершина
 диаграммы является центром окружности, проходящей через три сайта и
 расстояние от этой точки до заметающей прямой тоже равно радиусу этой
 окружности (это свойство точек, лежащих на «береговой линии»). Это —
 ключевой момент, потому что именно когда самая нижняя точка окружности,
 проходящей через три сайта, лежит ниже или на заметающей прямой, мы пушим
 в очередь событий событие круга с этой самой нижней точкой, потому что
 когда прямая на неё попадёт, мы получим вершину диаграммы Вороного.
*/
